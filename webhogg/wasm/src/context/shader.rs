use super::webgl;
use super::webgl::{ShaderType, WebGl2};
...
impl ShaderProgram {
    pub fn from_sources(gl: &WebGl2, sources: &[(ShaderType, String)]) -> Result<Self, WasmError> {
        let program = gl.create_program().map_err(|_| {
            WasmError::Shader(format!("glCreateProgram failed ({})", gl.get_error()))
        })?;
        for (shader_type, source) in sources {
            let shader = gl.create_shader(shader_type).map_err(|_| {
                WasmError::Shader(format!("glCreateShader failed ({})", gl.get_error()))
            })?;
            gl.shader_source(&shader, source);
            gl.compile_shader(&shader).map_err(|e| {
                WasmError::Shader(format!("compile error in {} shader: {}", shader_type, e))
            })?;
        ...
        if err.is_err() {
            Err(WasmError::Shader(format!(
                "program compile/link error: {}",
                err
            )))
        } else {
            Ok(Self { program })
